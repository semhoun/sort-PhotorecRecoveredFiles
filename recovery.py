...
def get_args():
    ...
    parser.add_argument('-n', '--max_per_dir', type=int, default=500, required=False, help='maximum number of files per directory')
    parser.add_argument('-m', '--months', action='store_true', required=False, help='split JPEG files not only by year but by month as well')
    parser.add_argument('-k', '--keep_filename', action='store_true', required=False, help='keeps the original filenames when copying')
    parser.add_argument('-e', '--delta', type=int, default=4, required=False, help='minimum delta in days between two days')
    parser.add_argument('-r', '--move', action='store_true', required=False, help='move file from source to dest')
    parser.add_argument("-d", "--days", action="store_true", help="create a subfolder per each different day (e.g. 20151230, 20151231, etc). Ignored if -e or -m is provided")
...
splitMonths = args.months
splitDays = args.days
keepFilename = args.keep_filename
minEventDeltaDays = args.delta
move = args.move

if splitDays:
    print("Reading from source '%s', writing to destination '%s' (max %i files per directory, splitting by day)." %
        (source, destination, maxNumberOfFilesPerFolder))
else:
...
imagesTYPE = ["JPG", "TIF", "DNG", "PEF", "NEF", "CR2", "ARW", "SR2"]
for ftype in imagesTYPE:
    dir = os.path.join(destination, ftype)
    if (os.path.exists(dir)):
        if splitDays:
            jpgSorter.postprocessImagesOneFolderPerDay(dir)
        else:
            jpgSorter.postprocessImages(dir, minEventDeltaDays, splitMonths)
